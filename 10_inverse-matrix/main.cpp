...
int main() {
  ...
  for (int i = 0; i < n; i++) matr_start[i] = new double[n];

  cout << "Enter the matrix's elements: " << endl;

  for (int i = 0; i < n; i++)
    for (int j = 0; j < n; j++) cin >> matr_start[i][j];

  cout << "Your recent matrix:" << endl;
  for (int i = 0; i < n; i++) {
    for (int j = 0; j < n; j++) cout << matr_start[i][j] << " ";
    cout << endl;
  }
  cout << endl;
  ...
  // СЛАУ
  cout << endl << "The system of equations." << endl;
